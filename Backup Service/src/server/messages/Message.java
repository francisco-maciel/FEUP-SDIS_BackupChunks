...
public abstract class Message {
	...
	public static Message parse(String message) throws UnrecognizedMessageException {
		...
		if (message == null) throw new UnrecognizedMessageException();	
		if ((parsed = parseHead(message)) == null) throw new UnrecognizedMessageException();
	...
	private static Message parseHead(String message) throws UnrecognizedMessageException {
		...
		try {
			...
			if (line.length() == 0) break; // empty line end header
			...
			if (lineIndex == 0) {
				...
				if (words.length != wordsByType.get(type)) return null;
		...
		if (lineIndex < 1) return null; // less than 1 line, not even header
		else {
			if (parsed.type.equals(MessageType.PUTCHUNK) ||
				...
				try {
					...
					else parsed.setBody(body.getBytes());
	...
	private static Message generateMessageHeader(MessageType type,
			String[] words) throws UnrecognizedMessageException {
		Message newMessage = null;
		try {
		if (type.equals( MessageType.PUTCHUNK))
			newMessage = new MessagePutChunk(words[1], words[2], Integer.parseInt(words[3]), null, Integer.parseInt(words[4]));
		else if (type.equals( MessageType.CHUNK))
			newMessage = new MessageChunk(words[1], words[2], Integer.parseInt(words[3]), null);
		else if (type.equals( MessageType.STORED))
			newMessage = new MessageStored(words[1], words[2], Integer.parseInt(words[3]));
		else if (type.equals( MessageType.GETCHUNK))
			newMessage = new MessageGetChunk(words[1], words[2], Integer.parseInt(words[3]));		
		else if (type.equals( MessageType.DELETE))
			newMessage = new MessageDelete(words[1]);
		else if (type.equals( MessageType.REMOVED))
			newMessage = new MessageRemoved(words[1], Integer.parseInt(words[2]));		
	...
	private static MessageType getMessageType(String header) throws UnrecognizedMessageException {
		if (header.equalsIgnoreCase("PUTCHUNK")) return MessageType.PUTCHUNK;
		else if (header.equalsIgnoreCase("CHUNK")) return MessageType.CHUNK;
		else if (header.equalsIgnoreCase("DELETE")) return MessageType.DELETE;
		else if (header.equalsIgnoreCase("GETCHUNK")) return MessageType.GETCHUNK;
		else if (header.equalsIgnoreCase("REMOVED")) return MessageType.REMOVED;
		else if (header.equalsIgnoreCase("STORED")) return MessageType.STORED;
